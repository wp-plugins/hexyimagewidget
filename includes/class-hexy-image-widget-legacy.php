 ...
 * @since 4.0.0
 */
class Hexy_Image_Widget_Legacy {
	/**
	 * Load legacy support.
	 *
	 * @since 4.0.0
	...
	public function load() {
		add_filter( 'hexy_image_widget_output', array( $this, 'output' ), 10, 4 );
		add_filter( 'hexy_image_widget_fields', array( $this, 'fields' ), 10, 2 );
		add_action( 'hexy_image_widget_field-legacy', array( $this, 'display_fields' ), 10, 2 );
		add_filter( 'hexy_image_widget_instance', array( $this, 'sanitize_data' ), 10, 4 );
	}
	...
	public function output( $output, $args, $instance, $id_base ) {
		if ( 'hexyimage' != $id_base || ! empty( $instance['image_id'] ) || empty( $instance['image'] ) ) {
			return $output;
		}
	...
	public function fields( $fields, $id_base ) {
		if ( 'hexyimage' == $id_base && is_hexy_image_widget_legacy() ) {
	...
	public function display_fields( $instance, $widget ) {
		if ( is_hexy_image_widget_legacy() || ! empty( $instance['image'] ) ) :
			?>
			<div class="hexy-image-widget-legacy-fields">
				<?php if ( ! is_hexy_image_widget_legacy() ) : ?>
					<p>
						<em><?php _e( 'These fields are here to maintain your data from an earlier version.', 'hexy-image-widget' ); ?></em>
					</p>
					<p>
						<em><?php _e( 'Select an image, then clear these values, and they will disappear when you save the widget.', 'hexy-image-widget' ); ?></em>
					</p>
				<?php endif; ?>

				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'image' ) ); ?>"><?php _e( 'Image URL:', 'hexy-image-widget' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'alt' ) ); ?>"><?php _e( 'Alternate Text:', 'hexy-image-widget' ); ?></label>
	 ...
	 *
	 * Called in Hexy_Image_Widget::update().
	 *
	 * @since 4.0.0
	 *
