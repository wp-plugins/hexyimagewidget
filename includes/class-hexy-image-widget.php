 ...
 * @since   3.0.0
 */
class Hexy_Image_Widget extends WP_Widget {
	/**
	...
	public function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		$id_base = ( $id_base ) ? $id_base : 'hexyimage'; // Legacy ID.
		$name    = ( $name ) ? $name : __( 'Hexy Image Widget', 'hexy-image-widget' );

		$widget_options = wp_parse_args(
			$widget_options,
			array(
				'classname'   => 'widget_hexyimage', // Legacy class name.
				'description' => __( 'Select an image from your Media Library.', 'hexy-image-widget' ),
			)
		);
	...
	public function widget( $args, $instance ) {
		$cache    = (array) wp_cache_get( 'hexy_image_widget', 'widget' );
		...
		$instance['rel_attribute'] = apply_filters( 'widget_text', $instance['rel_attribute'], $instance, $this->id_base );
		//$instance['rel_attribute'] = apply_filters( 'widget_text', $instance['text_raw'], $instance, $this->id_base );
		...
		if ( empty( $output ) ) {
			...
			if ( ! empty ( $instance['link'] ) ) {
				...
				$relatt = array('follow', 'nofollow');			
				$relattribute = 'rel="'. $relatt[$instance['rel_attribute']] .'"';

				$instance['link_open']  = '<a href="' . esc_url( $instance['link'] ) . '"' . $target . ' ' . $relattribute . '>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'hexy_image_widget', array_filter( $cache ), 'widget' );
		}
	}
	...
	public function render( $args, $instance ) {
		 ...
		 */
		$inside = apply_filters( 'hexy_image_widget_output', '', $args, $instance, $this->id_base );
		...
		if ( $inside ) {
			...
			$data['image_size'] = $image_size = ( ! empty( $instance['image_size'] ) ) ? $instance['image_size'] : apply_filters( 'hexy_image_widget_output_default_size', 'medium', $this->id_base );
			$data['title'] = ( empty( $instance['title'] ) ) ? '' : $instance['title'];
			$data = array_merge( $instance, $data );
			$data = apply_filters( 'hexy_image_widget_template_data', $data );

			ob_start();
			$templates = $this->get_template_names( $args, $instance );

			$template_loader = new Hexy_Image_Widget_Template_Loader();
	...
	public function form( $instance ) {
		...
		$button_class = array( 'button', 'button-hero', 'hexy-image-widget-control-choose' );
		 ...
		 * @param string $id_base Widget type id.
		 */
		$fields = (array) apply_filters( 'hexy_image_widget_fields', $this->form_fields(), $this->id_base );
		?>

		<div class="hexy-image-widget-form">
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'hexy_image_widget_form_before', $instance, $this->id_base );
			?>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'hexy-image-widget' ); ?></label>
			...
			<?php if ( ! is_hexy_image_widget_legacy() ) : ?>
				<p class="hexy-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Choose an Image', 'hexy-image-widget' ); ?>"
					data-update-text="<?php esc_attr_e( 'Update Image', 'hexy-image-widget' ); ?>"
					...
					?>
					<input type="hidden" name="<?php echo esc_attr( $this->get_field_name( 'image_id' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'image_id' ) ); ?>" value="<?php echo absint( $image_id ); ?>" class="image-id hexy-image-widget-control-target">
					<a href="#" class="<?php echo esc_attr( join( ' ', $button_class ) ); ?>"><?php _e( 'Choose an Image', 'hexy-image-widget' ); ?></a>
				</p>
			<?php endif; ?>
					...
					switch ( $field ) {
						case 'image_size' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'image_size' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php _e( 'Size:', 'hexy-image-widget' ); ?></label>
						...
						case 'rel_attribute' :
							$relatt = array('follow', 'nofollow');
							?>
							<p class="<?php echo esc_attr( $this->siw_field_class( 'rel_attribute' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'rel_attribute' ) ); ?>"><?php _e( 'Rel Attribute:', 'hexy-image-widget' ); ?></label>
								<select name="<?php echo esc_attr( $this->get_field_name( 'rel_attribute' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'rel_attribute' ) ); ?>" class="widefat image-size"<?php echo ( sizeof( $relatt ) < 2 ) ? ' disabled="disabled"' : ''; ?>>
									<?php
									foreach ( $relatt as $id => $follow ) {
										printf(
											'<option value="%s"%s>%s</option>',
											esc_attr( $id ),
											selected( $instance['rel_attribute'], $id, false ),
											esc_html( $follow )
										);
									}
									?>
						...
						case 'link' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php _e( 'Link:', 'hexy-image-widget' ); ?></label>
								<span class="hexy-image-widget-input-group">
									<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'link' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>" value="<?php echo esc_url( $instance['link'] ); ?>" class="hexy-image-widget-input-group-field">
									<button class="hexy-image-widget-find-posts-button hexy-image-widget-input-group-button dashicons dashicons-search"></button>
								</span>
							</p>
							<p class="<?php echo esc_attr( $this->siw_field_class( 'new_window' ) ); ?>" style="margin-top: -0.75em; padding-left: 2px">
								<label for="<?php echo esc_attr( $this->get_field_id( 'new_window' ) ); ?>">
									...
									<?php _e( 'Open in new window?', 'hexy-image-widget' ); ?>
								</label>
							</p>
							<?php
							break;
						...
						case 'link_classes' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_classes' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_classes' ) ); ?>"><?php _e( 'Link Classes:', 'hexy-image-widget' ); ?></label>
						...
						case 'link_text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_text' ) ); ?>"><?php _e( 'Link Text:', 'hexy-image-widget' ); ?></label>
						...
						default :
							 ...
							 * registered with the 'hexy_image_widget_fields'
							 * filter.
							 *
							 * @since 3.0.0
							 *
							 ...
							 */
							do_action( 'hexy_image_widget_field-' . sanitize_key( $field ), $instance, $this );
					}
				}
			}
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'hexy_image_widget_form_after', $instance, $this->id_base );
			?>

		</div><!-- /.hexy-image-widget-form -->
		<?php
	}
	...
	public function form_fields() {
		return array( 'image_size', 'link', 'link_text', 'link_classes', 'rel_attribute' );
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance = apply_filters( 'hexy_image_widget_instance', $instance, $new_instance, $old_instance, $this->id_base );
		...
		foreach ( array( 'link', 'link_classes', 'link_text', 'rel_attribute' ) as $key ) {
			...
			switch ( $key ) {
				...
				case 'link_text' :
					...
					break;
				case 'rel_attribute' :
					$instance['rel_attribute'] = stripslashes( wp_filter_post_kses( addslashes( $new_instance['rel_attribute'] ) ) );
					break;
			}
		}
	...
	public function get_image_sizes( $image_id ) {
		$sizes = array( 'full' => __( 'Full Size', 'hexy-image-widget' ) );

		$imagedata = wp_get_attachment_metadata( $image_id );
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = Hexy_Image_Widget_Plugin::get_image_size_names();
	...
	public function flush_group_cache( $post_id = null ) {
		...
		wp_cache_delete( 'hexy_image_widget', 'widget' );
	}
	...
	protected function siw_field_class( $id ) {
		$classes = array( 'hexy-image-widget-field', 'hexy-image-widget-field-' . sanitize_html_class( $id ) );

		$hidden_fields = Hexy_Image_Widget_Plugin::get_hidden_fields();
