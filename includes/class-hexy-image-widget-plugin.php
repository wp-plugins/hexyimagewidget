 ...
 * @since   3.0.0
 */
class Hexy_Image_Widget_Plugin {
	/**
	 * Set up the widget.
	 *
	 * @since 3.0.0
	...
	public function load() {
		...
		$compat = new Hexy_Image_Widget_Legacy();
		$compat->load();

		if ( is_hexy_image_widget_legacy() ) {
			return;
		}
		...
		add_action( 'wp_ajax_hexy_image_widget_find_posts', array( $this, 'ajax_find_posts' ) );
		add_action( 'wp_ajax_hexy_image_widget_preferences', array( $this, 'ajax_save_user_preferences' ) );
	}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'hexy-image-widget', false, dirname( dirname( plugin_basename( __FILE__ ) ) ) . '/languages' );
	}
	...
	public function register_widget() {
		register_widget( 'Hexy_Image_Widget' );
	}
	 ...
	 * 'hexy-image-widget-control' script a bit in order to allow it to be
	...
	public function register_assets() {
		wp_register_style(
			'hexy-image-widget-admin',
			dirname( plugin_dir_url( __FILE__ ) ) . '/assets/css/hexy-image-widget.css'
		);

		wp_register_script(
			'hexy-image-widget-admin',
			dirname( plugin_dir_url( __FILE__ ) ) . '/assets/js/hexy-image-widget.js',
		...
		wp_localize_script(
			...
			array(
				'l10n' => array(
					'frameTitle'      => __( 'Choose an Attachment', 'hexy-image-widget' ),
					'frameUpdateText' => __( 'Update Attachment', 'hexy-image-widget' ),
					'fullSizeLabel'   => __( 'Full Size', 'hexy-image-widget' ),
					'imageSizeNames'  => self::get_image_size_names(),
					'responseError'   => __( 'An error has occurred. Please reload the page and try again.', 'hexy-image-widget' ),
				),
	...
	public function widgets_screen_settings( $settings, $screen ) {
		...
		$settings .= sprintf( '<h5>%s</h5>', __( 'Hexy Image Widget', 'hexy-image-widget' ) );

		$fields = array(
			'image_size'   => __( 'Image Size', 'hexy-image-widget' ),
			'link'         => __( 'Link', 'hexy-image-widget' ),
			'link_classes' => __( 'Link Classes', 'hexy-image-widget' ),
			'link_text'    => __( 'Link Text', 'hexy-image-widget' ),
			'new_window'   => __( 'New Window', 'hexy-image-widget' ),
			'text'         => __( 'Text', 'hexy-image-widget' ),
		);
		 ...
		 */
		$fields = apply_filters( 'hexy_image_widget_hideable_fields', $fields );
		$hidden_fields = $this->get_hidden_fields();

		foreach ( $fields as $id => $label ) {
			$settings .= sprintf(
				'<label><input type="checkbox" value="%1$s"%2$s class="hexy-image-widget-field-toggle"> %3$s</label>',
				esc_attr( $id ),
	...
	public function enqueue_admin_assets() {
		wp_enqueue_media();
		wp_enqueue_script( 'hexy-image-widget-admin' );
		wp_enqueue_script( 'hexy-image-widget-find-posts' );
		wp_enqueue_style( 'hexy-image-widget-admin' );
	}
	...
	public static function get_image_size_names() {
		return apply_filters(
			...
			array(
				'thumbnail' => __( 'Thumbnail', 'hexy-image-widget' ),
				'medium'    => __( 'Medium', 'hexy-image-widget' ),
				'large'     => __( 'Large', 'hexy-image-widget' ),
				'full'      => __( 'Full Size', 'hexy-image-widget' ),
			)
		);
	}
	...
	public static function get_hidden_fields() {
		 ...
		 */
		return (array) apply_filters( 'hexy_image_widget_hidden_fields', $hidden_fields );
	}
	...
	public function ajax_find_posts() {
		...
		if ( ! $posts ) {
			wp_send_json_error( __( 'No items found.', 'hexy-image-widget' ) );
		}
	...
	protected function get_found_posts_html( $posts ) {
		$html = sprintf(
			...
			__( 'Title', 'hexy-image-widget' ),
			__( 'Type', 'hexy-image-widget' ),
			__( 'Date', 'hexy-image-widget' ),
			__( 'Status', 'hexy-image-widget' )
		);
		...
		foreach ( $posts as $post ) {
			...
			switch ( $post->post_status ) {
				case 'publish' :
				case 'private' :
					$status = __( 'Published', 'hexy-image-widget' );
					break;
				case 'future' :
					$status = __( 'Scheduled', 'hexy-image-widget' );
					break;
				case 'pending' :
					$status = __( 'Pending Review', 'hexy-image-widget' );
					break;
				case 'draft' :
					$status = __( 'Draft', 'hexy-image-widget' );
					break;
			}
			...
			$html .= sprintf(
				...
				esc_html( $title ),
				esc_url( apply_filters( 'hexy_image_widget_find_posts_post_link', $post_link ), $post->ID ),
	...
	public function print_find_posts_templates() {
		?>
		<script type="text/html" id="tmpl-hexy-image-widget-modal">
			<div class="hexy-image-widget-modal-head find-box-head">
				<?php _e( 'Find Post', 'hexy-image-widget' ); ?>
				<div class="hexy-image-widget-modal-close js-close"></div>
			</div>
			<div class="hexy-image-widget-modal-inside find-box-inside">
				<div class="hexy-image-widget-modal-search find-box-search">
					<?php wp_nonce_field( 'siw-find-posts', 'siw-find-posts-ajax-nonce', false ); ?>
					<input type="text" name="s" value="" class="hexy-image-widget-modal-search-field">
					<span class="spinner"></span>
					<input type="button" value="<?php esc_attr_e( 'Search', 'hexy-image-widget' ); ?>" class="button hexy-image-widget-modal-search-button" />
					<div class="clear"></div>
				</div>
				<div class="hexy-image-widget-modal-response"></div>
			</div>
		</script>
		<?php
	}
