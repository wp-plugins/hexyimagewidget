...
window.HexyImageWidget = window.HexyImageWidget || {};

(function( window, $, _, Backbone, wp, undefined ) {
	'use strict';

	var Control, l10n,
		HexyImageWidget = window.HexyImageWidget,
		Attachment = wp.media.model.Attachment,
		finder = { view: {} },
		frames = [];

	// Link any localized strings.
	l10n = HexyImageWidget.l10n = HexyImageWidget.l10n || {};
	...
	Control = function( el, options ) {
		...
		selector = this.$el.data( 'target' ) || '.hexy-image-widget-control-target';
		...
		this.frame = function() {
			...
			frame.state( 'library' ).on( 'select', function() {
				...
				frame.control.$el.trigger( 'selectionChange.hexyimagewidget', [ selection ] );
			});
	...
	_.extend( HexyImageWidget, {
		...
		getControl: function( el ) {
			...
			$control = $( el ).closest( '.hexy-image-widget-control' );
	...
	finder.view.Page = wp.Backbone.View.extend({
		events: {
			'click .hexy-image-widget-find-posts-button' : 'openModal'
		},
		...
		openModal: function( e ) {
			var $target = $( e.target).closest( '.hexy-image-widget-input-group' ).find( 'input[type="text"]' );
			e.preventDefault();
	...
	finder.view.Modal = wp.Backbone.View.extend({
		tagName: 'div',
		className: 'find-box',
		template: wp.template( 'hexy-image-widget-modal' ),

		events : {
			'click .hexy-image-widget-modal-search-button': 'findPosts',
			'keypress .hexy-image-widget-modal-search-field': 'routeKey',
			'keyup .hexy-image-widget-modal-search-field': 'routeKey',
			'click tr': 'select',
		...
		render: function() {
			...
			this.$field = this.$el.find( '.hexy-image-widget-modal-search-field' );
			this.$overlay = $( '.ui-find-overlay' );
			this.$response = this.$el.find( '.hexy-image-widget-modal-response' );
			this.$spinner = this.$el.find( '.hexy-image-widget-modal-search .spinner' );
			return this;
		},
		...
		findPosts: function() {
			...
			wp.ajax.post( 'hexy_image_widget_find_posts', {
				s: self.$field.val(),
	...
	$( document ).ready(function() {
		...
		$body.on( 'click', '.hexy-image-widget-control-choose, .hexy-image-widget-form img', function( e ) {
			e.preventDefault();
			HexyImageWidget.getControl( this ).frame().open();
		});

		// Update the image preview and size dropdown in a widget when an image is selected.
		$body.on( 'selectionChange.hexyimagewidget', '.hexy-image-widget-control', function( e, selection ) {
			var $control = $( e.target ),
				$sizeField = $control.closest( '.hexy-image-widget-form' ).find( 'select.image-size' ),
			...
			if ( $sizeField.length ) {
				HexyImageWidget.updateSizeDropdownOptions( $sizeField, sizes );
			}
			...
			$control.find( 'img' ).remove().end()
				.prepend( image )
				.addClass( 'has-image' )
				.find( 'a.hexy-image-widget-control-choose' ).removeClass( 'button-hero' );
		});

		// Wire up the toggle checkboxes in the screen options tab.
		$( '.hexy-image-widget-field-toggle' ).on( 'click', function() {
			var $this = $( this ),
				field = $this.val(),
				$hiddenFields = $( '.hexy-image-widget-field-toggle:not(:checked)' );

			$( '.hexy-image-widget-field-' + field ).toggleClass( 'is-hidden', ! $this.prop( 'checked' ) );

			$.ajax({
				url: ajaxurl,
				type: 'POST',
				data: {
					action: 'hexy_image_widget_preferences',
					hidden: $hiddenFields.map(function() { return this.value; }).get().join( ',' ),
					nonce: HexyImageWidget.screenOptionsNonce
				},
				success: function( data ) {
					if ( 'nonce' in data ) {
						HexyImageWidget.screenOptionsNonce = data.nonce;
					}
				}
			});
		});

		$body.on( 'focus', '.hexy-image-widget-input-group-field', function() {
			$( this ).parent().addClass( 'is-focused' );
		}).on( 'blur', '.hexy-image-widget-input-group-field', function() {
